<template>
    <div>
        ...
        <Input v-model="value" class="search" placeholder="请输入..." disabled icon="ios-search" style="width: 300px"></Input>
            ...
            <div style="text-align:center">
                <Form ref="myValidate" :model="myValidate" :rules="myRuleValidate" :label-width="80">
                    ...
                    <Form-item label="等级" prop="level">
                        <Input-number :max="101" :min="0" v-model="myValidate.level"></Input-number>
                    </Form-item>
        ...
        <Modal v-model="modal4" width="600px">
            <p slot="header" style="color:cornflowerblue;text-align:center">
                <Icon type="information-circled"></Icon>
                <span>操作</span>
            </p>
            ...
            <div slot="footer">
                <Button type="info" size="large" long :loading="modal4_loading" @click="submitForm2('myForm2')">点击确定</Button>
            </div>
        </Modal>
                            ...
                            return h('div', [
                                ...
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            this.modal4 = true;
                                            this.rolePermission(params.row.id);
                                            this.attachId = params.row.id;
//                                            this.deleteHid = params.row.id
                                        }
                                    }
                                }, '权限'),
                                h('Button', {
                ...
                myRuleValidate: {
                    name: [
                        ...
                        {max: 30, message: '角色名长度超出', trigger: 'blur'}
                    ],
                    displayName: [
                        {required: true, message: '角色别名不能为空', trigger: 'blur'},
                        {max: 30, message: '角色别名长度超出', trigger: 'blur'}
                    ],
                    level: [
                        {type: 'number', max: 101, message: '等级数字超出', trigger: 'blur'}
                    ],
                    description: [
                ...
                permission: [],
                attachId: '',//要分配权限的角色ID
                hadAttachPermission: [],//已经分配了权限的权限数组
                listStyle: {
                    width: '200px',
                    height: '500px',
                },
        ...
        methods: {
            ...
            rolePermission (id) {
                ...
                this.targetKeys2 = this.getTargetKeys(id)
            },
            getMockData () {
                let that = this;
                util.ajax({
                    url: '/console/permission',
                    method: 'get',
                    headers: {
                ...
                let dataLength = that.permission.length;
                let permissionList = that.permission;
                let mockData = [];
                for (let i = 0; i < dataLength; i++) {
//                    console.log(permissionList[i].name)
                    mockData.push({
                        key: permissionList[i].id,
                        label: permissionList[i].displayName,
                        description: permissionList[i].description,
                    });
                }
                return mockData;
            },
            getTargetKeys (id) {
                util.ajax({
                    ...
                    if (res.data.code == 0) {
//                        this.hadAttachPermission = res.data.data.permissionIds;
                        this.targetKeys2 = res.data.data.permissionIds;
                    } else {
                        this.$Message.error(res.data.message);
                        return false;
                    }
...
//                return this.hadAttachPermission;
            },
...
//                console.log(newTargetKeys);
            ...
            createNew () {
                ...
                this.myValidate.level = 1;
                this.myValidate.description = '';
            },
            submitForm2 () {
                var targetKeys2 = this.targetKeys2;
                var attachId = this.attachId;
                util.ajax({
                    url: '/console/role/' + attachId + '/cover/permission',
                    method: 'post',
                    headers: {
                        'x-auth-token': this.token,
                    },
                    data: {
                        'permissionIds':targetKeys2
                    }
                }).then((res) => {
                    if (res.data.code == 0) {
                        setTimeout(() => {
            ...
            submitForm (form) {
                ...
                this.$refs[form].validate((valid) => {
                    if (valid) {
                        ...
                        var level = that.myValidate.level;
                        var description = that.myValidate.description;
                        this.storeOrUpdate(name,displayName,level,description);
                    } else {
                        this.modal3_loading = false;
                    }
                })
            ...
            storeOrUpdate (name,displayName,level,description) {
                var doType = this.doType;

                if (doType == 'update') {
                    var id = this.editId;
                    var url = '/console/role/' + id;
                    var method = 'put'
                } else {
                    var url = '/console/role';
                    var method = 'post';
                }
